// COPY TO: frontend/src/app/api/vendor/profile/route.ts
...
// ✅ Get token from accessToken cookie or Authorization header
...
// ✅ Get vendor identity using local JWT decode
async function getVendorIdentity(
    ...
    // Allow VENDOR or ADMIN to access vendor profile routes
...
export async function GET(request: NextRequest) {
    try {
        const vendor = await getVendorIdentity(request);
        if (!vendor) {
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const current =
            index >= 0 ? data[index] : createDefaultProfileForVendor(vendor.userId, vendor.email);
